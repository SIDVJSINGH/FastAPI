...
class EmailsDAO:
    def __init__(self):
        self.table_name = "emails_flat"

    def query_emails_groupby(self, start_ts, end_ts, start_ts_lag, end_ts_lag, group_by_dim_str,
        ...
        where email_timestamp <= "{end_ts}" and email_timestamp >= "{start_ts}" {filter_condition}      
        ...
        from calcium-sunbeam-341614.rtp_solution.{self.table_name}  
        where email_timestamp <= "{end_ts_lag}" and email_timestamp >= "{start_ts_lag}" {filter_condition}    
        group by {group_by_dim_str}, {group_by_cols_tw_str}
        """

    def query_emails_total(self, start_ts, end_ts, start_ts_lag, end_ts_lag, group_by_cols_tw_str, lag=1,
                          filter_condition=''):
        return f"""
    ...
    def query_emails_top_10_by_dim_by_count(self, start_ts, end_ts, group_by_dim_str, filter_condition=''):
        if group_by_dim_str is None:
            return f"""with T as(
                        SELECT user_id,employee_id,employee_name,count(1) cnt
                        from calcium-sunbeam-341614.rtp_solution.{self.table_name}
                        where
                            email_timestamp >= "{start_ts}"
                            and email_timestamp <= "{end_ts}" {filter_condition}
                        group by 
    ...
    def get_groupby_data(self, start_ts, end_ts, start_ts_lag, end_ts_lag, group_by_dim_str, group_by_cols_tw_str,
        ...
        groupby_query = self.query_emails_groupby(start_ts, end_ts, start_ts_lag, end_ts_lag, group_by_dim_str,
    ...
    def get_total_data(self, start_ts, end_ts, start_ts_lag, end_ts_lag, group_by_cols_tw_str, lag,
                             filter_condition):
        total_query = self.query_emails_total(start_ts, end_ts, start_ts_lag, end_ts_lag,
                                                   group_by_cols_tw_str, lag, filter_condition)
    ...
    def get_top_10_by_dim_by_count_data(self, start_ts, end_ts, group_by_dim_str, filter_condition):
        top_10_query = self.query_emails_top_10_by_dim_by_count(start_ts, end_ts, group_by_dim_str,
                                                                     filter_condition)
        result = bq_client.query(top_10_query)
        return result


log_decorate_class(EmailsDAO)
