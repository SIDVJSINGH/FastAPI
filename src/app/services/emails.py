...
from app.processor.emails import EmailsProcessor



class EmailsService:
    def __init__(self, executor=None):
        self.emails_processor = None
    ...
    def _get_response_body(self, duration, start_date, end_date, company_id, user_id, view_filters , view, ):
        return {"data": {
            "name": "Emails",
    ...
    def get_all_data(self, request: APIInputBase):
        ...
        self.emails_processor = EmailsProcessor(time_window, current_ts, filter_condition)
        start_ts, end_ts = Utils.get_start_end_ts(current_ts, time_window)
        response_body= self._get_response_body(time_window, start_ts, end_ts, company_id, user_id, view_filters, view)
    ...
    def get_data_by_query(self, name, query, groupby_dimensions):
        ...
        if query == "groupby_query":
            df = self.emails_processor.process_groupby_query(groupby_dimensions)
            data_series = df.apply(lambda x: Utils.dim_data_to_dict(x, groupby_dimensions), axis=1)
        elif query == "total_query":
            df = self.emails_processor.process_total_query()
            data_series = df.apply(lambda x: Utils.dim_data_to_dict(x, groupby_dimensions), axis=1)
        elif query == "top_10_by_dim_by_count":
            df = self.emails_processor.process_top_n_by_dim_by_count_query(groupby_dimensions)
...
log_decorate_class(EmailsService)
