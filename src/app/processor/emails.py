...
from app.dao.emails_dao import EmailsDAO

emails_dao = EmailsDAO()
MODULE = 'emails'
...
class EmailsProcessor():
    ...
    def process_groupby_query(self, groupby_dimensions):
        ...
        query_result = emails_dao.get_groupby_data(self.start_ts, self.end_ts, self.start_ts_lag, self.end_ts_lag,
                                                        group_by_dim_str,
    ...
    def process_total_query(self):
        query_result = emails_dao.get_total_data(self.start_ts, self.end_ts, self.start_ts_lag, self.end_ts_lag,
                                                      self.group_by_cols_tw_str, self.lag, self.filter_condition)
        result_list = [dict(row) for row in query_result]
        df = pd.DataFrame(result_list)
        if df.shape[0] == 0:
    ...
    def process_top_n_by_dim_by_count_query(self, groupby_dimensions):
        if groupby_dimensions == []:
            ...
            group_by_dim_str = ','.join(groupby_dimensions)
        query_result = emails_dao.get_top_10_by_dim_by_count_data(self.start_ts, self.end_ts,
...
log_decorate_class(EmailsProcessor)